...
class KafkaStreamer:
    def __init__(self, bootstrap_servers, topic):
        ...
        if self.bootstrap_servers and "//" in self.bootstrap_servers:
            # Remove protocol prefix if present
            self.bootstrap_servers = self.bootstrap_servers.split("//")[-1]

    def connect(self):
        try:
            # Validate bootstrap server and topic
            if not self.bootstrap_servers or not self.topic:
                st.warning("Kafka bootstrap server or topic not configured. Streaming skipped.")
                return False
            ...
            st.error("Kafka library not found. Please install it with 'pip install kafka-python'.")
            return False
        except Exception as e:
